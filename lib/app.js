...
async function handleOrderStart({ command, client, respond }) {
  ...
  // 로깅 추가: 저장된 세션 정보 확인
  const savedSession = await orderManager.getSession(command.channel_id);
  logger.info('New session started with creator:', {
    channelId: command.channel_id,
    creatorId: command.user_id,
    savedCreatorId: savedSession.startedByUserId,
    messageTs: result.ts,
  });
}
