...
function formatOrderText(orderData, includeUserId = true) {
  const { userId, menu, temperature, beanOption, extraOptions, options } = orderData;
  ...
  // 메뉴 value를 text로 변환
  const menuText = menuConfig.menus.find((m) => m.value === menu)?.text || menu;
  orderParts.push(menuText);
...
async function handleOrderSubmission({ body, view, client }) {
  try {
    ...
    // 선택된 메뉴 정보 가져오기
    const selectedMenu = view.state.values.menu.menu_input.selected_option.value;
    
    // 메뉴 정보 찾기
    const menuItem = menuConfig.menus.find((m) => m.value === selectedMenu);
    
    // 커피 메뉴인지 확인
    const isCoffeeMenu = menuItem && menuConfig.categoriesNeedingBeanOption.includes(menuItem.category);
    
    // 원두 옵션 (커피 메뉴일 때만 기본값 설정)
    const beanOptionSelection = view.state.values.bean_option.bean_option_input.selected_option;
    ...
    const orderData = {
      userId: body.user.id,
      menu: selectedMenu,
      temperature: view.state.values.temperature.temperature_input.selected_option.value,
...
async function handleOrderStatus({ command, client, respond }) {
  ...
  for (const order of session.orders) {
    // 메뉴 value를 text로 변환 (정렬에 사용)
    const menuName = menuConfig.menus.find((m) => m.value === order.menu)?.text || order.menu;
